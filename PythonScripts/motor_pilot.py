...
motor_command_list = [[0, 50, 50, 4], [0, 50, 15, 3], [0, 50, 50, 4], [0, 15, 50, 3],]
...
def publish_motor_command(exit_loop): # subroutine
   if exit_loop == False: # Only do if user has not requested a stop
       ...
       print ("Direction: %d, Left: %d, Right: %d, Time: %.2f") % (motor_data.data[0], motor_data.data[1], motor_data.data[2], motor_data.data[3]) # print new motor speed on the terminal
       pub.publish(motor_data) # publish motor command to ROS
       while time.time() - loop_start < motor_data.data[3] and exit_loop == False: # keep the same values being sent to the motors for time specified or until key pressed
...
def main(argv):
    motor_data.data = [int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4])]
    ...
    pub.publish(motor_data) # publish motor command to ROS
    time.sleep(0.2) # make sure message has time to be enacted
